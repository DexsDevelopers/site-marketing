...
foreach($tables as $t) echo "- " . array_values($t)[0] . "<br>";
} catch (Exception $e) {
...
echo "Campanha 1 ativa: " . ($campanha['ativo'] ? 'SIM' : 'NÃO') . " (Limite: " . $campanha['membros_por_dia_grupo'] .
")<br>";
...
echo " <h4>Mensagens Pendentes para Envio AGORA: $total</h4>";
    ...
    $membros = fetchData($pdo, "SELECT id, ultimo_passo_id, data_proximo_envio FROM marketing_membros WHERE status =
    'em_progresso' LIMIT 10");
    foreach ($membros as $m) {
    $nextOrdem = $m['ultimo_passo_id'] + 1;
    $msgExists = fetchOne($pdo, "SELECT COUNT(*) as c FROM marketing_mensagens WHERE ordem = ? AND campanha_id = 1",
    [$nextOrdem])['c'];
    echo "- ID {$m['id']}: data_envio={$m['data_proximo_envio']}, próximo_passo={$nextOrdem}, msg_existe=" . ($msgExists
    ? 'SIM' : 'NÃO') . "<br>";
    }
    }
    ?>