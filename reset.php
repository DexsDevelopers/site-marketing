...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_bot'])) {
    ...
    if (curl_errno($ch)) {
        $message = 'Erro ao conectar com o bot: ' . curl_error($ch);
        ...
        if ($httpCode === 200 && isset($data['status']) && $data['status'] === 'success') {
            ...
            $message = '❌ Falha ao resetar: ' . ($data['message'] ?? 'Erro desconhecido');
            $messageType = 'alert-error';
        }
    }
    curl_close($ch);
...
<head>
    ...
    <style>
        ...
        .alert-success { background: rgba(0, 255, 0, 0.1); color: #4caf50; border: 1px solid #4caf50; }
        .alert-error { background: rgba(255, 0, 0, 0.1); color: #f44336; border: 1px solid #f44336; }
    </style>
</head>
<body>
    <div class="dashboard-container">
        ...
        <main class="main-content">
            ...
            <div class="panel">
                <div class="panel-title"><i class="fas fa-exclamation-triangle" style="color: #ff3b3b;"></i> Zona de Perigo</div>
                ...
                <div class="danger-zone">
                    ...
                    <p style="color: var(--text-dim); margin-bottom: 2rem; max-width: 600px; margin-left: auto; margin-right: auto;">
                        Esta ação irá derrubar a conexão atual com o WhatsApp, apagar os dados de sessão (pasta <code>auth_info_baileys</code>) e reiniciar o bot forçadamente.
                        <br><br>
                        <strong>Use isso apenas se precisar escanear um novo QR Code ou se o bot estiver travado.</strong>
                    </p>

                    <form method="POST" onsubmit="return confirm('Tem certeza absoluta? Isso irá desconectar o bot do WhatsApp!');">
